...
getData(request)
    .then(data => {
        ...
        //sort value
        data = data.sort((a,b) => {return d3.ascending(a.value2017, b.value2017)})
        ...
        Array.from(label.nodes()).forEach((el)=>{
            el.htmlFor === "year0" ? el.innerHTML=nameValuet0 : el.innerHTML=nameValuet1
        })
    ...
    //Axis scale
    const xmin = Math.min(
        d3.min(data, (d)=>{return +d.value2013}),
        d3.min(data, (d)=>{return +d.value2017})
    )
    const xmax = Math.max(
        d3.max(data, (d)=>{return +d.value2013}),
        d3.max(data, (d)=>{return +d.value2017}))
    ...
    const y = d3.scaleBand()
        ...
        .domain(data.map((d) => { return d.libreg; }))
        .paddingInner(0.3)
    ...
    g.append("text")
        ...
        bars.join(
            enter => enter.append("rect")
                .attr("x", 0)
                .attr("y", (d) => {return y(d.libreg)})
                .style("fill", "#b1b1b1")
                .attr("height", y.bandwidth())
                .call(enter => enter.transition(tBars)
                .attr("width", (d) => {return x(d["value2017"])})),
            
            update => update
                .attr("fill", "black")
                .call(update => update.transition(tBars)),
            ...
            .on('mouseover', (d, i, arr) => {
                ...
                const widthTooltip = document.querySelector("#tooltip-barchart").clientWidth
                const heightTooltip = document.querySelector("#tooltip-barchart").clientHeight
                ...
                d3.select("#tooltip-barchart")
                    ...
                    <p>${nameValuet0}</p><p>${numberFormat.format(d.value2013)} ${unit}</p>
                    </div>
                    <div class="tooltip-item">
                    <div>${nameValuet1}</div><div>${numberFormat.format(d.value2017)} ${unit}</div>
                    </div>
                    `)
        ...
        const value0 = d3.extent(data, (d)=>{return +d.value2013})
        const value1 = d3.extent(data, (d)=>{return +d.value2017})
        const extentValue0 = value0[1]-value0[0];
        const extentValue1 = value1[1]-value1[0];
                
        extraBarchartValue
                .html(`${extentValue1} ${unit}`)
        ...
        buttons.on("change", function(d){
            ...
            if (selection === "A"){
                d3.selectAll("rect")
                .transition(tBars)
                .attr("width", (d) => {return x(d["value2013"])})

                extraBarchartValue
                .html(`${extentValue0} ${unit}`)
            } else{
                d3.selectAll("rect")
                .transition(tBars)
                .attr("width", (d) => {return x(d["value2017"])})
